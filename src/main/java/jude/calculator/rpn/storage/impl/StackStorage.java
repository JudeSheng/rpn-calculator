package jude.calculator.rpn.storage.impl;
...
import jude.calculator.rpn.storage.MementoIF;
...
public class StackStorage implements Storage {
	...
	public boolean store(Double number) {
		if(number == null || number.isInfinite() || number.isNaN()) {
			...
			stack.push(RpnCalculatorUtils.round(number, storageDecimal));
			return true;
		}
	}
	...
	public String toString() {
		...
		stack.stream().forEach(number -> {
			String str = String.valueOf(RpnCalculatorUtils.round(number, displayDecimal));
